...
<div style="font-size: 1.2em; overflow-x: auto;">
  <table border="1" cellpadding="8" cellspacing="0" style="border-collapse: collapse; width: 100%;">
    ...
    <tbody>
      <tr>
        <td style="text-align: center;">res_simple_o2</td>
        <td style="text-align: center;">77289339928</td>
      ...
      <tr>
        <td style="text-align: center;">res_simple_o3</td>
        <td style="text-align: center;">78278906748</td>
      </tr>
      <tr>
        <td style="text-align: center;">res_vector_o2</td>
        <td style="text-align: center;">21437829214</td>
      </tr>
      <tr>
        <td style="text-align: center;">res_vector_o3</td>
        <td style="text-align: center;">21502081699</td>
      </tr>
      <tr>
      ...
      <tr>
        <td style="text-align: center;">res_quad_o3</td>
        <td style="text-align: center;">53175028722</td>
...
Обратим внимание на значение Simple/Quad (подсвечено красным). Это отношение близко к единице, то есть получилось, что Quad-версия практически не ускорила программу, что очень странно. Чтобы разобраться с тем, почему Quad версия такая медленная, я зашёл на godbolt.org, чтобы посмотреть, в какие ассемблерные инструкции компилятор переводит код моей функции.
...
Мы видим в коде много ветвлений и обращений в память, что говорит о том, что компилятору не удалось векторизовать вычисления. Возникло предположение, что компилятор не может векторизовать код вида "for if".
Для этого используем хитрый ход: заменим ветвление на арифметику, дающую тот же результат. Сделаем это следующим образом:
```
for (int i = 0; i < kVecWidth; i++) if (cmp[i]) x_arr[i] = x2[i] - y2[i] + x0_arr[i]; else x_arr[i] = x_arr[i];
for (int i = 0; i < kVecWidth; i++) if (cmp[i]) y_arr[i] = xy[i] + xy[i] + y0_arr[i]; else y_arr[i] = y_arr[i];
```
...
Видим явные изменения в ассемблерном коде. Сократилось количество обращений в память и компилятор смог векторизовать вычисления.
  ...
  <table border="1" cellpadding="8" cellspacing="0" style="border-collapse: collapse; width: 100%;">
    ...
    <tbody>
      <tr>
        ...
        <td style="text-align: center;">30870120448</td>
      ...
      <tr>
        ...
        <td style="text-align: center;">78678878055</td>
      </tr>
    </tbody>
  </table>
</div>
...
Это произошло из-за того, что компилятору просто не хватило регистров для хранения пачки из восьми двоек. Чтобы решить эту проблему, нужно произвести следующую перестановку кода:
```
                int mask = 0;
                for (int i = 0; i < kVecWidth; i++) mask |= cmp[i] << i;
                if (!mask) break;
...
## Интересные факты
Когда запускаем тест С ГРАФИКОЙ на ФИКСИРОВАННОМ ядре, получается пиздец, потому что два потока борются за ядро (расчет и отрисовка).
Когда запускаем тест с ГРАФИКОЙ без фиксированного ядра, планировщик распределяет один поток на одно ядро, а другой на другое и получается быстрее, чем первый вариант.

Когда запускаем тест БЕЗ ГРАФИКИ на ФИКСИРОВАННОМ ядре, всё хорошо считается.
Когда запускаем тест БЕЗ ГРАФИКИ БЕЗ ФИКСИРОВАННОГО ядра, он работает дольше, потому что когда планировщик при переходе с одного ядра на другое, кэши инвалидируются.
